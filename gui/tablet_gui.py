...
class TabletGUI:
    ...
    async def process_order(self, beverage_name: str, quantity: int, price: float, prep_time: float):
        ...
        try:
            # Create order - status: PENDING (Sent)
    ...
    def _update_order_in_tree(self, order):
        ...
        def update():
            ...
            self.status_label.config(
                text=f"⚙️  Active: {active_orders} | Total: {len(all_orders)} | API: http://localhost:8080"
            )
    ...
    def on_naruchi_click(self):
        ...
        # Get base price from Factory-created beverage object
        base_price = beverage_obj.price()
        print(f"Base price from Factory: {beverage_obj.__class__.__name__} = {base_price} EUR")
