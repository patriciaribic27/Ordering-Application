...
Includes fallback mechanism and robust error handling for stability.
"""
...
# Try to import reportlab with version checking
try:
    import reportlab
    ...
    print(f"✓ ReportLab {REPORTLAB_VERSION} loaded successfully")
except ImportError as e:
    REPORTLAB_AVAILABLE = False
    REPORTLAB_VERSION = None
    print(f"⚠️  ReportLab not available: {e}")
    print("   PDF export will create simple text files instead")
except Exception as e:
    REPORTLAB_AVAILABLE = False
    REPORTLAB_VERSION = None
    print(f"⚠️  ReportLab import error: {e}")
    print("   PDF export will use fallback mode")

from services.order_service import Order, OrderStatus


class PDFExporter:
    """Exporter for PDF format reports with fallback mechanism."""
    ...
    def __init__(self, export_dir: str = "izvjestaji"):
        ...
        # Initialize reportlab styles if available
        if REPORTLAB_AVAILABLE:
            try:
                self._init_styles()
            except Exception as e:
                print(f"⚠️  Error initializing PDF styles: {e}")
                self.reportlab_available = False
    
    def _init_styles(self):
        """Initialize reportlab styles (only if reportlab is available)."""
    ...
    def export_daily_report(self, orders: List[Order]) -> str:
        """
        Export daily report with statistics to PDF (with fallback).
        ...
        Raises:
            Exception: If both PDF and fallback export fail
        """
        if not self.reportlab_available:
            print("⚠️  ReportLab unavailable, using text fallback")
            return self._export_text_fallback(orders)
        
        try:
            return self._export_pdf_report(orders)
        except Exception as e:
            print(f"⚠️  PDF export failed: {e}")
            print("   Falling back to text export...")
            return self._export_text_fallback(orders)
    
    def _export_text_fallback(self, orders: List[Order]) -> str:
        """
        Fallback: Export report as formatted text file when PDF fails.
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Total Revenue:     {total_revenue:.2f} EUR\n")
            
            # Statistics per beverage
            f.write("\n" + "-"*70 + "\n")
            f.write("STATISTICS PER BEVERAGE\n")
            f.write("-"*70 + "\n\n")
            ...
            f.write(f"{'Beverage':<15} {'Orders':<10} {'Quantity':<10} {'Revenue':>15}\n")
            f.write("-"*70 + "\n")
            
            for beverage, stats in sorted(beverage_stats.items(), 
                                         key=lambda x: x[1]['revenue'], 
                                         reverse=True):
                f.write(f"{beverage:<15} {stats['count']:<10} {stats['quantity']:<10} {stats['revenue']:>12.2f} EUR\n")
            
            # Detailed orders
            f.write("\n" + "-"*70 + "\n")
            f.write("DETAILED ORDERS\n")
            f.write("-"*70 + "\n\n")
            
            f.write(f"{'ID':<10} {'Beverage':<15} {'Qty':<5} {'Price':<10} {'Status':<12} {'Time':<10}\n")
            f.write("-"*70 + "\n")
            
            for order in sorted(orders, key=lambda x: x.created_at):
                status_text = order.status.value.upper()
                f.write(f"{order.id[:8]:<10} {order.beverage_name:<15} {order.quantity:<5} "
                       f"{order.price:>6.2f}€   {status_text:<12} {order.created_at.strftime('%H:%M:%S'):<10}\n")
            
            f.write("\n" + "="*70 + "\n")
            f.write(f"Generated by Modern Ordering Application • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("="*70 + "\n")
        
        print(f"✓ Text report created: {filepath}")
        return str(filepath)
    
    def _export_pdf_report(self, orders: List[Order]) -> str:
        """
        Internal method: Export report as PDF using reportlab.
